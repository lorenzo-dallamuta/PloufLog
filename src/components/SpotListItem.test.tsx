...
describe('MySpots - SpotListItem', () => {
  ...
  it('displays the dive site location', () => {
    render(<SpotListItem item={mockDiveSite}/>);
    expect(screen.getByText(mockDiveSite.data.properties.country_iso3)).toBeTruthy();
  });

  it('displays the dive average rating', () => {
    render(<SpotListItem item={mockDiveSite}/>);
    expect(screen.getByText(mockDiveSite.data.properties.averageRating)).toBeTruthy();
  });
